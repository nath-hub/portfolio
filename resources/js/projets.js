...
// Utiliser les projets de la BD s'ils existent, sinon utiliser les données statiques
const projects = window.projectsData && window.projectsData.length > 0 ? window.projectsData : defaultProjects;

// Vérifier et ajouter les slugs manquants aux projets de la BD
projects.forEach((project, index) => {
    if (!project.slug) {
        // Générer un slug à partir du titre si absent
    ...
    if (!project.category) {
        project.category = 'web'; // catégorie par défaut
    }
});
                    ...
                    <a href="${projectUrl}" class="project-link">
                        <div class="project-image">
                            ...
                            <div class="project-tags">
                                ${project.tags && project.tags.map ? project.tags.map((tag) => `<span class="tag">${tag}</span>`).join("") : ''}
                            </div>
                        </div>
                        <div class="project-content">
                            ...
                            <div class="project-footer">
                                <div class="tech-stack">
                                    ${project.tech && project.tech.map ? project.tech.map((tech) => `<div class="tech-icon" title="${tech}">${tech.slice(0, 2).toUpperCase()}</div>`).join("") : ''}
                                </div>
                            </div>
                        </div>
                    </a>
                    <div class="project-actions">
                        <a href="${projectUrl}" class="view-btn">Voir Détails →</a>
                    </div>
                </div>
            `;
}
